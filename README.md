...
2. Добавь обязательный `challenge.json`:
```json
{
  "name": "My Task",
...
| Поле | Статус | Описание | По умолчанию |
|---|---|---|---|
| `name` | опционально | Название челленджа в CTFd | имя папки таски |
| `description` | опционально | Описание задачи | пусто |
| `category` | опционально | Категория в CTFd | имя папки категории |
| `value` | опционально | Количество очков | `100` |
| `flag` | опционально | Флаг (можно также в `flag.txt` рядом с таской) | — |
| `connection_info` | опционально | Строка подключения (URL таски) | пусто |
| `type` | опционально | Тип челленджа CTFd | `standard` |
| `state` | опционально | `visible` / `hidden` | `visible` |
| `tags` | опционально | Массив тегов | пусто |
| `files` | опционально | Массив путей к файлам (относительно папки таски) | пусто |

Обязателен только сам файл `challenge.json` — без него таска не публикуется. Остальные поля можно опускать, их значения подставятся из умолчаний.

Файлы челленджа (например zip-архивы) можно указать двумя способами:
- поле `"files"` — массив путей относительно папки таски (как выше);
- подпапка `files/` рядом с таской — все файлы из неё загрузятся автоматически.

Оба способа работают вместе. Файлы загружаются в CTFd через API и появляются на странице челленджа.
